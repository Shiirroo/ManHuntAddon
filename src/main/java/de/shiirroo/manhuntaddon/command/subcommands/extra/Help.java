    ...
    public void perform(Player p, String[] args) {
        if(args.length == 1){
            ...
            if((5 * page) < getSubCommands.size()) {
                ...
                p.sendMessage(ChatColor.WHITE +"--- "+ChatColor.AQUA+"Information about "+ ManHuntAddon.getPlugin().getName()+ ChatColor.GOLD+ "v" + ManHuntAddon.getPlugin().getDescription().getVersion() +ChatColor.WHITE +" - "+ChatColor.GREEN +"Page " +ChatColor.GOLD+ (page + 1)+ChatColor.WHITE+ " | "  +ChatColor.GOLD+  ((getSubCommands.size()) / Math.min(getSubCommands.size(), 5)) +ChatColor.WHITE+" ---");
